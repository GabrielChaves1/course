...
func NewResetPassword(idpClient idp.Client) *ResetPassword {
	return &ResetPassword{
		idpClient: idpClient,
	}
}

func (uc *ResetPassword) Execute(ctx context.Context, cmd request.ResetPasswordDTO) error {
	...
	if err := uc.idpClient.ResetPassword(ctx, email.Value, code.Value, password.Value); err != nil {
